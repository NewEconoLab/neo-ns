...
![](media/bbca8b4e45de22f8ea00bf8462f0f309.png)
...
拍卖注册器通过静态调用appcall 调用nnc合约处理用户充值与提现（拍卖费用）

NNC合约
-------

NNC合约是NNS系统的代币，他主要是一个拓展了NEP5功能实现的代币

有两大块拓展

1.  Nep5.1扩展（支持代币的循环分配，分期领奖的扩展）

2.  Nep5.5扩展（支持转账证明和智能合约交易的扩展）
...
Nnc合约是个nep5合约，可以用对nep5合约的验证方法验证
...
2.  修改拍卖注册期代码中nnccall指向的合约地址为nnc合约地址
